...
c          -- Cells in the same XY sheet -
           lx2 = lx+1
           if(lx2.le.ncx)then
             call pre_celij_MLS(j1,j1+ncall1,kind_p1,ini_kind_p2,lx2)     !East
           endif      
           lz2=lz+1
           if(lz2.le.ncz)then       
             !- Same row -
             call pre_celij_MLS(j1,j1+ncall2,kind_p1,ini_kind_p2,lx2)   !North       
             lx2=lx+1
             if(lx2.gt.2) call pre_celij_MLS(j1,j1+ncall4,
     &                                 kind_p1,ini_kind_p2,lx2)   !North-West       
             lx2=lx+1
             if(lx2.le.ncx) call pre_celij_MLS(j1,j1+ncall3,
...
c      -- Periodic Boundary Calls in X-Direction --              
       if(i_periodicOBs(1).eq.1)then
         ...
         do lz = 1,ncz-1
           ...
           call pre_celij_MLS(j1,j1+ncall12,kind_p1,ini_kind_p2,lx2) !North-East X-Periodic
         ...
         do lz = 1,ncz-1
           ...
           call pre_celij_MLS(j1,j1+ncall13,kind_p1,ini_kind_p2,lx2) !North-West X-Periodic
         end do    !End of:  do lz = 1,ncz-1       
