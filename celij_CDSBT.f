         ...
         if (nc(j2,kind_p2).ne.0) then 
            ...
            do ii=1,nc(j1,kind_p1)
               ...
               do jj = 1,nc(j2,kind_p2)  
                  ...
                  jp = ibox(j2,kind_p2,jj)      ! jp is neighbour particle
              
                  drx = xp(ip) - xp(jp)
                  drz = zp(ip) - zp(jp)

                  call periodicityCorrection(ip,jp,drx,drz,lx2)
...
!!!                  Calculating kernel & Normalized Kernel Gradient
                     ...
                     call kernel(drx,drz,ip,jp,j1,j2,rr2) 
...
!                     if (ip.gt.nvmb.and.jp.gt.nvmb) then   ! Both fluid particles or
                                                           ! virtual moving boundary particles
                     if (itype(ip).ge.2.and.itype(jp).ge.2) then   
     ...
     +                  robar,one_over_rhobar,ip,jp,j1,j2,term2i,term2j)
...
!                     else if (jp.gt.nvmb.and.ip.le.nvmb) then     
                     else if (itype(jp).ge.2.and.
     &                        abs(itype(ip)).le.1) then   
...
!!!                     DENSITY ACCELERATION (1992; 3.9)
                     ...
                     else if (abs(itype(ip)).eq.1.and.
     &                        abs(itype(ip)).eq.1) then   
...
!                           az(ip) = az(ip) - pm(jp) * p_v * frzi
                           ...
                           call viscosityCDSBT(dot,drx,drz,-dux,-duz,
     +                                rr2,jp,ip,term2i,term2j)
