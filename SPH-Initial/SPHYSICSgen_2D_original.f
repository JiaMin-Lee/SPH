...
!     _______ water in the inclined region
     ...
     +               g,dx,dy,dz,expont,beta,0.)
      ...
      open(77,file='velocityinlet')

!    VELOCITY INLET/OUTLET
       write(*,*) 'Add velocity inlet/outlet ?? (1=y)'
       read(*,*) i_velocityopt
       write(*,*) i_velocityopt
       if (i_velocityopt.gt.0) then
           write(77,*) 1
           write(77,*) i_velocityopt
       endif

       do while(i_velocityopt.gt.0)
        call virtualMB(nn,dx,dy,dz,beta,0.)
        write(*,*) 'Add another velocity inlet/outlet ?? (1=y)'
        read(*,*) i_velocityopt
        write(*,*) i_velocityopt
        if (i_velocityopt.gt.0) then        
            write(77,*) i_velocityopt
...
!            N = N_finish
            ...
            N_start=N0
            N_finish=N-1
        else if(i_geometry.ne.5.and.i_geometry.ne.2)then
           N_start = 0
           N_finish = N-1
        end if 
...
!       if (i_geometry.ne.5.or.i_geometry.ne.2)then
!          N_start = 0
!          N_finish = N
!        else
!          N_start=N0
...
c	____________________ SUBROUTINE GATE
      ...
      subroutine virtualMB(nn,dx,dy,dz,beta,theta)
      include 'common.gen2D'

      double precision Xwall,Xlength
...
!          L_end = L_end - 1
  	   ...
  	   do j=M_ini,M_end
            ...
            if(i_velocityopt.eq.1) then
            call pos_veloc(nn,((N_ini-n)*dx),j*dy,k*dz,VXgate,0.,VZgate)
            else if (i_velocityopt.eq.2) then
            call pos_veloc(nn,((N_ini+n)*dx),j*dy,k*dz,VXgate,0.,VZgate)
            endif
  	   ...
  	   do j=M_ini,M_end
	      ...
	      call pos_veloc(nn,(Xwall-n*dx),j*dy,k*dz,0.,0.,0.)
              ...
              ! condition
                ...
                if (iBC.eq.1.and.i_velocityopt.lt.1) then  
             !2-D Normals
                   if(k.eq.L_ini)then
                     i_minus1 = nn + 1
                     i_plus1  = nn 
...
c     -- Read in Floating Body parameters --
     ...
     &          *(radiuscircle(num_FB)*radiuscircle(num_FB))
       bigInertiaYY(num_FB) = bigMass(num_FB)
     &          *(radiuscircle(num_FB)**2)/2               !area moment of inertia         
...
!     &           + dz_Box*cos(body_Angle(num_FB))
         ...
         u1=bigU(num_FB) - radiuscircle(num_FB)*bigOmega(num_FB)
     &                     *sin(pi-angle)
         w1=bigW(num_FB) + bigOmega(num_FB)*radiuscircle(num_FB)
     &                     *cos(pi-angle)
         call pos_veloc(nn,x1,y1,z1,u1,v1,w1)
!         call pressure(nn,ZZmax,dx,dy,dz,expont,g)
